...
export default async function ActivityDetailsPage({
  ...
  try {
    // First try finding by slug (handling both raw and decoded forms just in case)
    const result = await payload.find({
      collection: 'activities',
      where: {
        or: [
          { slug: { equals: id } },
          { slug: { equals: decodedId } }
        ]
      },
      limit: 1,
    });
    ...
    if (result.docs.length > 0) {
      ...
      // MongoDB document IDs are strings, so use the route value unchanged.
  ...
  const imageUrl = (activity.coverImage as any)?.url;
  ...
  return (
    <main className="flex-grow flex flex-col w-full">
      ...
      <section className="relative w-full bg-on-background min-h-[35vh] flex items-center justify-center overflow-hidden pt-stack-lg">
        ...
        <div className="relative z-10 w-full max-w-container-max-width px-margin-mobile md:px-margin-desktop text-center flex flex-col items-center gap-stack-md py-stack-lg">
          <div className="inline-flex items-center gap-2 bg-saffron/20 text-saffron px-3 py-1 rounded-full border border-saffron/30 backdrop-blur-sm">
            ...
            <span className="text-label-md font-label-md uppercase tracking-wider">Activity</span>
          </div>
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-lg border border-outline-variant overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter p-margin-mobile md:p-margin-desktop pt-8 md:pt-12">
            ...
            <article className="lg:col-span-9 prose prose-lg max-w-none text-on-surface font-body-lg text-body-lg 
              ...
              {activity.content ? (
                <RichText data={activity.content as any} />
              ) : (
                <p>No content provided for this activity.</p>
              )}
            </article>
          </div>
        </div>
      ...
      {recentActivities.docs.length > 0 && (
        <section className="w-full max-w-container-max-width mx-auto px-margin-mobile md:px-margin-desktop py-stack-lg border-t border-outline-variant">
          <div className="flex justify-between items-center mb-stack-md">
            ...
            <Link href="/activities" className="text-primary font-label-md text-label-md hover:underline flex items-center gap-1">
              View All <span className="material-symbols-outlined text-[16px]">arrow_forward</span>
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter">
            {recentActivities.docs.map((item: any) => {
              const itemImageUrl = (item.coverImage as any)?.url;
              return (
                <Link key={item.id} href={`/activities/${item.slug || item.id}`} className="group block bg-surface-container-lowest rounded-lg border border-outline-variant overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-4">
                    <div className="text-caption font-caption text-on-surface-variant mb-2 flex items-center justify-between">
